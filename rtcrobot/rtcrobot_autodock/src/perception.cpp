#include <ros/ros.h>
#include <rtcrobot_autodock/icp_2d.h>
...
void
DockPerception::createSample()
{
  // Create ideal cloud
  // Front face is 350mm long
  for (double y = 0.0; y <= 0.163; y += 0.001)
  {
  ...
  // Each side is 100mm long, at 30 degree angle
  for (double y = 0; y <= 0.0866 /*0.0707106*/; y += 0.001)
  {
    geometry_msgs::Point p;
    p.x = y*0.57735026919;
    p.y = 0.163 + y;
  ...
  for (double y = 0.0866; y > 0 /*0.0707106*/; y -= 0.001)
  {
    geometry_msgs::Point p;
    p.x = y*0.57735026919;
    p.y = 0.335 - y;
  ...
  for (double y = 0.335; y <= 0.385; y += 0.001)
  {
    geometry_msgs::Point p;
    p.x = p.z = 0.0;
    p.y = y;
...
DockPerception::stop()
{
  running_ = false;
  found_dock_ = false;
  dock_ = geometry_msgs::PoseStamped();
  return true;
}
...
bool DockPerception::getPose(geometry_msgs::PoseStamped& pose, std::string frame)
{
  ...
  if (ros::Time::now() > dock_stamp_ + ros::Duration(0.5))
  {
    ROS_WARN("Dock pose timed out");
...
void 
DockPerception::callback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
      ...
      (dock_.pose.orientation.z == 0.0 && dock_.pose.orientation.w == 0.0))
  {
    ...
    for (size_t i = 3*scan->ranges.size()/4; i < scan->ranges.size(); i++)
    {
      if (std::isfinite(scan->ranges[i]))
      {
        ...
        ROS_INFO("Set initial pose to (%f, %f, %f)",
  ...
  // Sap xep dua vao khoang cach cuoi cung cua dock
...
DockCandidatePtr DockPerception::extract(laser_processor::SampleSet* cluster)
{
  ...
  {
    ROS_WARN("Couldn't transform laser point");
    return candidate;
  }
  ...
  if (fitness >= 0.0)
  {
    ...
    while (retry-- &&
            (
              ...
              fabs(angles::normalize_angle(tf::getYaw(tf::inverse(cand_pose)*init_pose))) > 3.1415/2.0 //4.0
            )
          )
    {
      // Try one more time.

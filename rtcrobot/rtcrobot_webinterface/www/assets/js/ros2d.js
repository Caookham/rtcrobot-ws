...
ROS2D.Viewer.prototype.shift = function(x, y) {
  ...
  console.log("k1: " + this.scene.y)
  console.log("k2: " + this.scene.scaleY)
...
/*----------------------------Shift Custom-----------------------------------*/
/*****************************************************************************/
ROS2D.Viewer.prototype.shift_custom = function(x, y) {
  // save current offset
  this.scene.x_prev_shift = this.scene.x;
  this.scene.y_prev_shift = this.scene.y;
  // shift scene by scaling the desired offset
  this.scene.x = x;
  this.scene.y = y;
  console.log("hello: " + this.scene.x)
};
/*****************************************************************************/
...
*   * background (optional) - the color to render the background, like '#efefef'
*/
ROS2D.Viewer_custom = function(options) {
  var that = this;
  options = options || {};
  var divID = options.divID;
  this.width = options.width;
  ...
  this.scene.scale = 1;
  // create map container and robot container
...
/*****************************************************************************/
/*------------------------Scale To Dimensions Custom-------------------------*/
/*****************************************************************************/
ROS2D.Viewer_custom.prototype.scaleToDimensions_custom = function(widthof_canvas, widthof_map, heightof_map, zoom) {

  this.map.scaleX = widthof_canvas / widthof_map * zoom;
  this.map.scaleY = this.map.scaleX;
  this.canvas.width = widthof_canvas;
  this.canvas.height = heightof_map * this.map.scaleX;


};
/*****************************************************************************/

ROS2D.OccupancyGridClient_custom = function(options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
  var topic = options.topic || '/map';
  ...
  rosTopic.subscribe(function(message) {
      ...
      this.currentGrid = new ROS2D.OccupancyGrid_custom({
          message: message
      });
...
ROS2D.OccupancyGridClient_custom.prototype.__proto__ = EventEmitter2.prototype;

ROS2D.OccupancyGrid_custom = function(options) {
  options = options || {};
  var message = options.message;
...
*   * width - width of robot, in meter
*   * height - height of robot, in meter
*   * resolution - 
*   * background (optional) - the color to render the background, like '#efefef'
*/
ROS2D.Add_Robot = function(options) {
  var that = this;
  options = options || {};
  this.width_m = options.width || 100;
  this.height_m = options.height || 100;
  var background = options.background || '#C0C0C0';
  this.resolution = options.resolution || 1;
  this.width_p = this.width_m / this.resolution;
  this.height_p = this.height_m / this.resolution;
  this.rootObject = options.rootObject || new createjs.Container();

  this.icon_robot = new createjs.Shape();
  this.icon_robot.graphics.clear();
  this.icon_robot.graphics.beginStroke("rgba(0,0,0,0.5)");
  this.icon_robot.graphics.beginFill('rgba(255,0,0,0.7)')
  this.icon_robot.graphics.drawRoundRect(0 - this.width_p / 2, 0 - this.height_p / 2, this.width_p, this.height_p, 2);
  this.icon_robot.graphics.endStroke();

  this.arror = new createjs.Shape();
  this.arror.graphics.clear();
  //this.arror.graphics.beginStroke("rgba(0,0,0,0.5)");
  this.arror.graphics.beginFill('#FFFFFF')
  this.arror.graphics.drawPolyStar(this.width_p / 4, 0, this.width_p / 5, 3, 0, 0);
  this.arror.graphics.endStroke();

  this.rootObject.addChild(this.icon_robot, this.arror);
